...
# F40=c(0.033,.033) #I dont actually know what is F40. F40% The fishing mortality at which spawning biomass-per-recruit is 40% of its unfished level..2389 from Spies Punt
 ...
 for (y in 2:Nyrs){
  ...
  #Try 10% migration from P1 to P2 and the other way at the age 0 phase (larval)
   #It seems most realistic to have it for just age 1s  
#   if (MIG==1){ #one way 1 to 2
#     Natage_P1_Nyrs[y,1]=VBRec_P1*(1-m) #m is migration rate from 1 to 2
#     Natage_P2_Nyrs[y,1]=VBRec_P2+(VBRec_P1*m)
#     migrants_to2[y]=VBRec_P1*m/sum(Natage_P2_Nyrs[y,])#how many is 2 getting relative to what it had
#     migrants_to1[y]=0/sum(Natage_P1_Nyrs[y,])#none
#   }
   
 #  if (MIG==2){ #two way
#     Natage_P1_Nyrs[y,1]=VBRec_P2*m+VBRec_P1*(1-m);
#     Natage_P2_Nyrs[y,1]=VBRec_P1*m+VBRec_P2*(1-m);
#     migrants_to2[y]=VBRec_P1*m/sum(Natage_P2_Nyrs[y,])#how many is 2 getting relative to what it had. denominator is already adjusted
#     migrants_to1[y]=VBRec_P2*m/sum(Natage_P1_Nyrs[y,])#how many is 1 getting relative to what it had
#   } #number in P2 that move to P1 plus the ones that do not leave P1.
   
#   if (MIG==3){   #one way 2 to 1
#     Natage_P1_Nyrs[y,1]=VBRec_P1+VBRec_P2*m #m is migration rate from 2 to 1
#     Natage_P2_Nyrs[y,1]=VBRec_P2*(1-m)
#     migrants_to2[y]=VBRec_P2*m/sum(Natage_P1_Nyrs[y,])#how many is 2 getting relative to what it had
#     migrants_to1[y]=0/sum(Natage_P2_Nyrs[y,])#none
#   }   #Natage_P1_Nyrs[y,1]=VBRec_P1;
   
    
   #this is migration at all life stages
...
#  }
 ...
 pcor_raw=cor.test(rowSums(P1_raw[251:Nyrs,]),rowSums(P2_raw[251:Nyrs,]),alternative="greater")
...
# plot(rowSums(P2_raw[51:Nyrs, ]),type="l",ylim=c(0,max(rowSums(P2_raw[51:Nyrs, ]))));lines(rowSums(P2_srvsel[51:Nyrs, ]),col="red")
 ...
 outpops[[7]]=rowSums(t(t(Natage_P1_Nyrs)*Wt_all)) #t(t(a)*b)
 outpops[[8]]=rowSums(t(t(Natage_P2_Nyrs)*Wt_all)) 
 return(outpops)
}
...
for(exp in 1:1){#was 1:11
 ...
 for(popstat in 1:3){  #was 1:4
  ...
  setwd("/Users/ingrid.spies/Documents/GOA_cod/SimulationResults/COV_2pops_SM_forcesizes_02_age0/")
  ...
  for (k in 1:Nrep){  #k is number of Nrep
   for(j in 1:length(m)){   #m is the different migration rates you are testing
    ...
    cor2=sort(cor);
    #If statement identifies whether none are significant (integer(0) means none are below significance threshold)
    #If there are some that are below significance threshold, the else statement gathers those values.
...
#    Biomass=c(test[[7]],test[[8]])
#   Name=c(rep("PopA",300),rep("PopB",300))
#    Res=data.frame(Biomass,Name,Year)
#    print(ggplot(Res)+geom_line(aes(x=Year,y=Biomass,col=Name))+theme_bw())
#Plot number of fish   
 #   Year=c(rep(seq(1,Nyrs,1),2))
...
Res=data.frame(Number,Name,Year)
ggplot(Res)+geom_line(aes(x=Year,y=Number,col=Name))+theme_bw(base_size)
