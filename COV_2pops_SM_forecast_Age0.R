...
MIG=2;
  ...
  setwd("/Users/ingrid.spies/Documents/GOA_cod/SimulationResults/COV_2pops_SM_forecast_age1/")
...
#mean(Number[350:400])/434
#mean(Number[750:800])/750
...
mig=seq(0,0.28,.01)
forecast_age1_F06=cbind(mig,WAI,CAI)
write.csv(forecast_age1_F06,"forecast_age1_F06.csv") #done.

mig=seq(0,0.28,.01)
forecast_age1_F02=cbind(mig,WAI,CAI)
write.csv(forecast_age1_F02,"forecast_age1_F02.csv")



print(ggplot(Res)+geom_line(aes(x=Year,y=Number,col=Population))+theme_bw(base_size=16))+
  geom_ribbon(aes(x=Year,ymin = LCI, ymax = HCI,fill=Population),alpha=0.4)+
  ylab("Female Spawning Biomass (t)")+ylim(c(0,2000))+
  geom_hline(yintercept=0.4*750,lty=1)+geom_hline(yintercept=0.4*434,lty=2)
