...
for(exp in 1:1){#was 1:11
 ...
 for(popstat in 1:3){  #was 1:4
  ...
  setwd("/Users/ingrid.spies/Documents/GOA_cod/SimulationResults/COV_2pops_SM_forcesizes_06_age0/")
...
#I did one iteration of test=
#MIG=2
#m[j]=0.1

Year=c(rep(seq(1,Nyrs,1),2))
Number=c(test[[7]],test[[8]])
Name=c(rep("PopA",Nyrs),rep("PopB",Nyrs))
Res=data.frame(Number,Name,Year)
ggplot(Res)+geom_line(aes(x=Year,y=Number,col=Name))+theme_bw(base_size=14)+ylab("Female spawning biomass (t)")+
    geom_hline(yintercept = 434*.4,lty=2)+
    geom_hline(yintercept=750*.4)+geom_vline(xintercept=150)
